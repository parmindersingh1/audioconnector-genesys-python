...
// fixed test-client.ts (only changed message handler and minor typings)
import WebSocket from "ws";
...
class AudioTester {
  ...
  async connect() {
    ...
    this.ws = new WebSocket("ws://localhost:8080/ws", {
      headers: {
        "X-API-KEY": "gx_4k8n2m9p6r3s7t1v5w8x2y4z6a9b3c7d1e5f8g2h6i0j4k7l1m9n3o6p2q8r5s1t7u4v0w6x9y3z8",
        "audiohook-session-id": "test-audio-session-123",
        "audiohook-organization-id": "test-org-456",
        "audiohook-correlation-id": "test-correlation-789",
      },
    ...
    // NOTE: use the (data, isBinary) signature to reliably detect binary frames
    this.ws.on("message", (data: WebSocket.RawData, isBinary: boolean) => {
      if (isBinary) {
        // Binary audio data — do NOT attempt to JSON.parse
        const buf = Buffer.isBuffer(data) ? data : Buffer.from(data as any);
        this.audioReceived.push(buf);
        console.log(
          `Received audio: ${buf.length} bytes (total: ${this.audioReceived.length} chunks)`
        );
        return;
      }

      // Non-binary - treat as text (JSON). Protect with try/catch.
      const txt = data.toString();
      let message: AudioMessage | null = null;
      try {
        message = JSON.parse(txt) as AudioMessage;
      } catch (err) {
        console.warn("Received non-JSON text on websocket (ignored):", txt);
        return;
      }
  ...
  private startAudioTest() {
    ...
    // Generate test audio: alternating patterns to simulate speech
    const testDuration = 3000; // 3 seconds
    const chunkSize = 160; // 20ms chunks at 8kHz (PCMU bytes)
    const chunksPerSecond = 50; // 1000ms / 20ms
    const totalChunks = (testDuration / 1000) * chunksPerSecond;
    ...
    const audioInterval = setInterval(() => {
      ...
      for (let i = 0; i < chunkSize; i++) {
        const sample = Math.sin((chunksSent * chunkSize + i) * 0.1) * 50 + 128;
        audioChunk[i] = Math.floor(sample) & 0xff;
      }
